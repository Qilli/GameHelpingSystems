...
namespace Base.Curves
{
    ...
    public class CurvedPath : ICurve
    {
        ...
        public Vector3 getPointOnCurve(float t)
        {
            float remapedTime;
            int startCurve;
            getStartIndexForLocalCurve(t, out remapedTime, out startCurve);
        ...
        public Vector3 getCurveTangentAt(float t)
        {
            ...
            int startCurve;
            getStartIndexForLocalCurve(t, out remapedTime, out startCurve);
            Vector3 result = getTangentOnLocalCurve(pathPoints[startCurve++], pathPoints[startCurve++], pathPoints[startCurve++], pathPoints[startCurve], remapedTime);
            return result;
        }

        public Quaternion getCurveOrientationAt(float t)
        { 
            return Quaternion.LookRotation(getCurveTangentAt(t));
        }
        ...
        {
            Vector3 d, e;
            computePartBezierLerp(p0, t0, t1, p1, t, out d, out e);
            //d-e
            return Vector3.Lerp(d, e, t);
        }

        private static void computePartBezierLerp(Vector3 p0, Vector3 t0, Vector3 t1, Vector3 p1, float t, out Vector3 d, out Vector3 e)
        {
            //lerp a-b,b-c,c-d
        ...
        private Vector3 getTangentOnLocalCurve(Vector3 p0, Vector3 t0, Vector3 t1, Vector3 p1, float t)
        {
            Vector3 d, e;
            computePartBezierLerp(p0, t0, t1, p1, t, out d, out e); 
            //d-e
        ...
        private void getStartIndexForLocalCurve(float t, out float remapedTime, out int startCurve)
        {
            ...
            int selectedSegment = Mathf.Clamp(Mathf.FloorToInt(t * segments), 0, segments-1);
            //remap t for selected segment
            float segmentSize = 1.0f / segments;
            remapedTime = Base.CommonCode.MathHelpers.MathHelpers.Remap(selectedSegment * segmentSize, selectedSegment * segmentSize + segmentSize, 0.0f, 1.0f, t);
            //get point on local curve
            startCurve = selectedSegment * 3;
        }
        #endregion
